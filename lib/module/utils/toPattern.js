...
function toPattern(value, optionPattern) {
  ...
  for (index = 0; index < outputLength; index++) {
    ...
    if (charCounter >= values.length) {
      ...
      break;
    } else if (output[index] === DIGIT && values[charCounter].match(/\d/) || output[index] === DIGIT_SPACE && values[charCounter].match(/[\d\s]/) || output[index] === DIGIT_SPACE_PLUS_MINUS && values[charCounter].match(/[\d\s+-]/) || output[index] === ALPHA && values[charCounter].match(/[a-zA-Z]/) || output[index] === ALPHA_SPACE && values[charCounter].match(/[a-zA-Z\s]/) || output[index] === ALPHANUM && values[charCounter].match(/[a-zA-Z0-9]/) || output[index] === ALPHANUM_SPACE && values[charCounter].match(/[a-zA-Z0-9\s]/) || output[index] === CHAR && values[charCounter].match(/[\S]/) || output[index] === CHAR_SPACE && values[charCounter].match(/./) || output[index] === HOURS && values[charCounter].match(/[0-23]/) || output[index] === MINUTES && values[charCounter].match(/[0-59]/) || output[index] === SECONDS && values[charCounter].match(/[0-59]/) || output[index] === SECONDS && values[charCounter].match(/[0-59]/)) {
      ...
      console.log('###5', 'invalid character retry with next character', {
        output,
        index,
        placeholder,
        thisSlice: output.slice(0, index).join('')
      });
  ...
  console.log('###8', output.join('').substr(0, index), {
    output,
    index
  });
